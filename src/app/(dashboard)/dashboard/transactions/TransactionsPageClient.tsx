...
export function TransactionsPageClient() {
  ...
  const transactionQueryParams = useMemo(
    () => normalizeTransactionListQueryParams(searchParams),
    [searchParams]
  );

  const filters = useMemo<FilterOptions>(() => {
    return {
      type: transactionQueryParams.type,
      categoryId: transactionQueryParams.categoryId,
      accountId: transactionQueryParams.accountId,
      startDate: transactionQueryParams.startDate,
      endDate: transactionQueryParams.endDate,
    };
  }, [transactionQueryParams]);

  const page = transactionQueryParams.page ?? DEFAULT_TRANSACTION_PAGE;
  const pageSize = transactionQueryParams.pageSize ?? DEFAULT_TRANSACTION_PAGE_SIZE;
  const sortBy = transactionQueryParams.sortBy ?? DEFAULT_SORT_BY;
  const sortOrder = transactionQueryParams.sortOrder ?? DEFAULT_SORT_ORDER;
